...
if(isset($_GET['file']))
{
	...
	else
	{
		...
		{
			if($flas !==false && $flags=="no_compress")
			{
			...
			else
			{
				...
				switch(strtoupper($configs['jscript_comressor_type']))
				{
					case "JAVASCRIPTPACKER":
						$packer = new JavaScriptPacker(makeDirRelative($res, $_relativeImgDir), $configs['jscript_encoding'], 
						$configs['jscript_fast_decode'], $configs['jscript_special_chars']);
